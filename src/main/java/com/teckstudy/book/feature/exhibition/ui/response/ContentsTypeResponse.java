package com.teckstudy.book.feature.exhibition.ui.response;

import com.teckstudy.book.feature.exhibition.application.dto.ContentsDto;
import com.teckstudy.book.feature.exhibition.types.ContentEnum;
import lombok.Builder;
import lombok.Getter;

@Getter
@Builder
public class ContentsTypeResponse {

    private Long exhibitionId;
    private Long contentId;
    private ContentEnum contentEnum;
    private int contentCnt;

    /**
     * 전시카테고리 컨텐츠 조회
     */
    public ContentsTypeResponse(Long exhibitionId, Long contentId, ContentEnum contentEnum, int contentCnt) {
        this.exhibitionId = exhibitionId;
        this.contentId = contentId;
        this.contentEnum = contentEnum;
        this.contentCnt = contentCnt;
    }

    public static ContentsTypeResponse from(ContentsDto contentsType) {
        return new ContentsTypeResponse(
                contentsType.getExhibitionId(),
                contentsType.getContentId(),
                contentsType.getContentEnum(),
                contentsType.getContentCnt()
        );
    }

}